...
export default function ActivePenalties() {
  ...
  if (activePenalties.length === 0) {
    return (
      <div className="text-center py-4" style={{ color: theme.colors.neutral[500] }}>
        Inga aktiva straff
      </div>
    )
  }
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          onMouseEnter={(e) => {
            e.currentTarget.style.backgroundColor = theme.components.button.secondary.backgroundHover
          }}
          onMouseLeave={(e) => {
          ...
          >
            <div className="flex justify-between items-start">
              <div className="flex-1">
                <p className="font-medium" style={{ color: theme.components.penalty.activeText }}>
                  {penalty.description}
                </p>
                <div className="flex items-center gap-4 mt-2 text-sm" style={{ color: theme.components.penalty.activeText }}>
                  <span>Spelare: {getPlayerName(penalty.targetPlayerId || '')}</span>
                  <span 
                    className="px-2 py-1 rounded"
                    style={{ 
                      backgroundColor: penalty.type === 'shot' ? theme.components.penalty.shot : 
                                     penalty.type === 'hole' ? theme.components.penalty.hole : 
                                     theme.components.penalty.persistent,
                      color: theme.colors.neutral[800]
                    }}
                  >
                    {penalty.type === 'shot' ? 'Nästa slag' : 
                     penalty.type === 'hole' ? 'Detta hål' : 
                     'Bestående'}
                  </span>
                </div>
              </div>
      ...
      >
        <h4 className="font-semibold mb-2" style={{ color: theme.colors.blue[800] }}>
          Guide för straffvaraktighet
        </h4>
        <div className="text-sm space-y-1" style={{ color: theme.colors.blue[700] }}>
          <div><strong>Nästa slag:</strong> Tillämpas på nästa slag som tas</div>
          <div><strong>Detta hål:</strong> Tillämpas för hela det aktuella hålet</div>
          <div><strong>Bestående:</strong> Förblir aktivt tills spelet slutar</div>
        </div>
      </div>
    </div>
  )
